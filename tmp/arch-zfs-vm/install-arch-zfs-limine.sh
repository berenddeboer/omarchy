...
ZFS_DEVICE=/dev/disk/by-id/virtio-omarchy-zfs-root-part2
...
if ! pacman -Si zfs-linux-lts >/dev/null 2>&1; then
  printf 'Required ArchZFS package zfs-linux-lts is unavailable from the live ISO pacman config.\n' >&2
  exit 1
fi
